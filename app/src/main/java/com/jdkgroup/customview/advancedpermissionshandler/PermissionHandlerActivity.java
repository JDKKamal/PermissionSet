package com.jdkgroup.customview.advancedpermissionshandler;
...
public abstract class PermissionHandlerActivity extends AppCompatActivity implements ActivityCompat.OnRequestPermissionsResultCallback {
    ...
    private String messageSwitcherForToast() {
        return customMessage == null ? MessageGenerator.makeToastDialogMessage(convertListToArray(remainedPermissionsList), this) : customMessage;
    }

    //if custom message is null. so generate auto message for alert dialog
    @NonNull
    private String messageSwitcherForDialog() {
        return customMessage == null ? MessageGenerator.makeAlertDialogMessage(convertListToArray(remainedPermissionsList), this) : customMessage;
    }
    ...
    private void initialize(String[] permissions, @Nullable String message, boolean sticky, PermissionCallBack
            mPermissionCallBack) {
        this.mActivity = this;
    ...
    private void showPermissionDialog(final String[] permissions) {
        ...
        ).setPositiveButton("OK", (dialog, which) -> askForPermission(permissions)).setNegativeButton("cancel", (dialog, which) -> {
            if (mSticky) {
                openSettingsForPermission();
            } else {
                if (permissionCallBack != null) {
